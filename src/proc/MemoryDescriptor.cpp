...
void MemoryDescriptor::NMapToPageTable()
{
	...
	int j = 1;
	int textcnt = (m_TextSize + 4096 - 1) / 4096;
	int datacnt = (m_DataSize + 4096 - 1) / 4096;
	int stackcnt = (m_StackSize + 4096 - 1) / 4096;

	for (int i = 0; i < textcnt; i++) {
		pUserPageTable[1].m_Entrys[j].m_Present = 1;
		pUserPageTable[1].m_Entrys[j].m_ReadWriter = 0;
		pUserPageTable[1].m_Entrys[j].m_PageBaseAddress = i + (textAddress >> 12);
		j++;
	}
	...
	for (int i = 0; i < datacnt; i++) {
		...
		pUserPageTable[1].m_Entrys[j].m_PageBaseAddress = 1 + i + (u.u_procp->p_addr >> 12);
		j++;
	}

	for (int i = 0; i < stackcnt; i++) {
		pUserPageTable[1].m_Entrys[1023 - i].m_Present = 1;
		pUserPageTable[1].m_Entrys[1023 - i].m_ReadWriter = 1;
		pUserPageTable[1].m_Entrys[1023 - i].m_PageBaseAddress = 1 + datacnt + i + (u.u_procp->p_addr >> 12);
	}
